...
| `NginxEdgeConfigTest` (8 tests, 39 assertions) | installs, skips reload when unchanged, refuses a broken host config, restores on rejection and on unreloadable config, requires `nginx` when a destination is set, and that `deploy.sh` sources/calls/undoes correctly | each case asserts on the file on disk and a reload marker produced by a stub `nginx`; the stub deliberately errors if it is not told which file to validate, because a stub that greps an empty path calls every config valid and all three refusal tests then pass vacuously. The case set also caught a leak of the author's own making: it ran with the ambient `PATH`, so it passed in a clean checkout and **failed inside the full suite** on a session whose `PATH` contained a rehearsal stub named `nginx` — every case now runs with `PATH` set to a temporary directory holding only the tools the helper needs, with or without the stub, which also stops a host `systemctl` from silently changing the branch taken |
