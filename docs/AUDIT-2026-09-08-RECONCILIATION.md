...
| Data volume / performance envelope | "no N+1 / no unbounded queries" is asserted from review, not measured against realistic volumes. One measured signal from this session's live run, recorded rather than interpreted: three endpoints took ~500 ms while 14 consoles fired a concurrent burst — `/api/v1/management` 500.36 ms, `/api/v1/identity/people` 500.91 ms, `/api/v1/identity/accounts` 501.07 ms (all HTTP 200; the E2E gate still reported 25/25 succeeded, 0 failed). `grep -rn "usleep\|sleep(" app/` finds **no** artificial delay in application code, so the two candidate explanations are dev-server queueing under `artisan serve`'s single worker or real per-endpoint cost — which is precisely what the review-only claim cannot distinguish. | run the reporting and identity journeys with seeded realistic data behind the production topology, and record query counts and p95 latencies |
