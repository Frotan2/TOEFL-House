...
**Two findings the measurement produced, one fixed:** 8 of the 30 queries on every authenticated API request were `employments` lookups, because `AccessResolution` asked one HR eligibility question at four places per decision — memoized per decision (not per request: the container binding is a singleton, so only that lifetime is correct under both FPM and a long-running worker), taking the request to 24 queries and p50 from 17.4 ms to 14.7 ms with every authorization decision unchanged; and `tests/Feature/Access/` had **no mention of employment eligibility at all**, i.e. a deny path in the most-tested module was uncovered — now 5 behaviour tests plus a cost rule. The second finding is a scaling law no row-count test can see: `Controller::authorizedBranches()` resolves authority once per active branch, measured at **13 queries per branch** (24 / 37 / 76 / 167 queries at 1 / 2 / 5 / 12 branches), pinned as a slope test so a per-request memo may land but a per-branch query may not; collapsing the loop itself is recorded as an authorization decision for `docs/05-SECURITY-RBAC-GOVERNANCE.md`, not a performance-gate change. | closed for the identity read surface — `tests/Feature/Performance/ReadPathEnvelopeTest.php` and `scripts/runtime/perf-envelope.php` are committed, so the property is now re-measured by CI instead of asserted from review; see `docs/AUDIT-2026-09-08-GATE-EVIDENCE.md` §Gate F. **Still open, and why:** the students/reporting journeys cannot be amplified by copy (`students_one_per_person` + `students_one_per_admission_decision` + `students_admission_authority_guard()` require a real final-admit decision per row) and the legal builder costs 110 ms/student, so 1,000-row volumes are not measured in the default suite; there is no performance SLO in the repository to certify against, so the committed rules assert shape (flat query count, bounded payload) rather than speed; no sustained load generator exists — 8-way curl bursts are a queueing probe only |
