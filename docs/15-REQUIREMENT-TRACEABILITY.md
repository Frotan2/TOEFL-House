...
| R-07 | Effective campus assignments cannot overlap | 04, 06 | migration `000170` | `BranchTransferFeatureTest`; schema-invariant test | **Verified (focused 2026-09-10)** — direct writes receive SQLSTATE `23P01` from `campus_assignments_no_effective_overlap`; this is distinct from the four production-table races in `verify:concurrency` |
| R-08 | Transactional outbox and idempotent consumers | 09 | `Outbox/*`, `Integrations/*` | outbox/consumer tests | **Verified in suite**; the supervised relay entrypoint fails closed until a reviewed `INTEGRATIONS_SCHEDULER_RUN_BY` and one host timer/cron supervisor are configured (`operations/production-deployment.md` §10) |
