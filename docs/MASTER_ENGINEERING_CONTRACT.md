...
docs/08-PEOPLE-HR-CRM-OPERATIONS.md
...
The canonical documentation system is entered through `docs/README.md`. The Master Engineering Contract is the permanent engineering constitution. Current domain and operational rules are maintained in the canonical root documents linked by the index. Historical archive material is evidence only.

The Constitution governs what must remain true; the Ultimate Goal defines the destination; the current architecture and domain manuals define how the platform is governed today; testing/release documents define evidence; the current-state register defines what exists and what remains.
