...
**Authority SHA at audit start:** `17c6605d42a19f678150174dd2f241a8c7c878e8`  
...
| Access | roles, permissions, grants, delegations, scoped assignments | Access workspace | Covered/Partial | Surface complete approval/revoke/delegation workflows |
...
| Reporting | metric definitions/versions, report runs, dashboards, projections/reconciliation | Reporting workspace | Covered/Partial | Complete run lifecycle and evidence/status actions |
...
## 6. Frontend-only responsibilities
...
5. Documentation names the capability and identifies whether it is covered, partial, missing or backend-only.
6. Browser verification confirms the intended workspace mounts without credential leakage or asset-path regressions.

## 8. Release gates

The repository is **not certified complete merely because the capability matrix exists**. Release certification requires the authoritative `main` branch to pass the Verification workflow, including frontend, static analysis, backend tests and browser E2E, with no unresolved critical/high parity or security defects.
...
The capability map should be regenerated/reviewed whenever a domain route or React workspace changes materially.
