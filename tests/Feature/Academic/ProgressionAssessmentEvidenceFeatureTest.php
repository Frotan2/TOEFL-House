...
final class ProgressionAssessmentEvidenceFeatureTest extends CanonicalTestCase
{
    public function test_progression_rejects_an_assessment_result_that_is_not_released(): void
    {
        ...
        $chain = $this->newAcademicChain($officer, 'progev');
        app(MaintainAcademicStructure::class)->defineLevel(
            $officer,
        ...
        $level = ProgramVersionLevel::query()->whereKey($class['program_version_level_id'])->firstOrFail();
        ...
        try {
            app(DecideProgression::class)->propose(
                ...
                'progev-propose',
                $result->id,
                'released evidence required for advancement',
            );
