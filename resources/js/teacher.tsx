...
function TeacherApp({ getJson, postJson, csrfToken }: TeacherApi) {
  ...
  const addQualification = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!selected || Object.values(qualification).some((value: string) => value.trim() === '')) return;
  ...
  if (loading) return <PageStatus>Resolving teacher capability, employment, provenance, and assignment facts…</PageStatus>;
