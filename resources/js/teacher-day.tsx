  ...
  return <>
    ...
    <main id="workspace-main" className="workspace teacher-day" aria-labelledby="teacher-day-title">
      <header className="workspace-header day-header">
        <div><p className="eyebrow">People & Faculty · Daily workspace</p><h1 id="teacher-day-title">Teach the day, not the database.</h1><p className="lede">{profile?.legal_name ?? 'Teacher'} · scheduled sessions are matched against effective class assignments from the canonical People and Academic projections.</p></div>
      ...
      <section className="day-brief" aria-label="Teacher daily brief">
        <div className="day-brief-main"><span className="eyebrow">Daily brief</span><strong>{sessions.length === 0 ? 'No teaching sessions are scheduled for this date.' : `${sessions.length} teaching session${sessions.length === 1 ? '' : 's'} scheduled.`}</strong><span>{profile?.professional_title ?? 'Faculty member'} · {totalMinutes / 60 || 0} teaching hours projected · {academic.generated_at}</span></div>
      ...
      <section className="day-layout">
        <section className="panel" aria-labelledby="day-schedule-heading">
          <div className="section-heading"><div><p className="eyebrow">Today / selected date</p><h2 id="day-schedule-heading">Teaching schedule</h2></div><span className="source-note">{sessions.length} session{sessions.length === 1 ? '' : 's'} · source-owned timetable</span></div>
