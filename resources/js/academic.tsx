...
function AcademicHeader({ csrfToken }: { csrfToken: string }) {
  return <AppShell current="academic" csrfToken={csrfToken} />;
}
...
export function AcademicApp({ getJson, postJson, csrfToken }: AcademicProps) {
  ...
  if (loading && data === null) return <><AcademicHeader csrfToken={csrfToken} /><PageStatus>Loading authorized academic scope…</PageStatus></>;
  if (!data) return <><AcademicHeader csrfToken={csrfToken} /><main id="workspace-main" className="workspace"><div className="alert" role="alert">{error ?? 'No academic workspace projection is available.'}</div></main></>;

  return <>
<AcademicHeader csrfToken={csrfToken} />
    <main id="workspace-main" className="workspace" aria-labelledby="academic-title">
