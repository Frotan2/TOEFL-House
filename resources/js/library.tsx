...
export function LibraryApp({ getJson, postJson, csrfToken }: ApiClient & { csrfToken: string }) {
  ...
  const counts = [
    ...
    ['Pending disposal', workspaceData.disposal_requests.filter((d) => !['completed', 'executed', 'cancelled', 'withdrawn'].includes(String(d.lifecycle_state))).length],
