...
export function CrmApp({ getJson, postJson, csrfToken }: CrmAppProps) {
  ...
  const capture = (event: FormEvent<HTMLFormElement>) => {
    ...
    void postJson('/crm/visitors', body, requestKey(slot)).then(() => { clearRequestKey(slot); setForm(emptyCrmForm); setMessage('Visitor captured in the CRM source of truth.'); loadVisitors(); }).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'The visitor could not be captured.')).finally(() => setSaving(false));
  };
