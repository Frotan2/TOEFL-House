...
class ApiError extends Error {
  ...
  constructor(status: number, payload: ApiErrorPayload, fallback: string) {
    ...
    const message = payload.message ?? (code ? code : fallback);
...
async function readApiResponse<T>(response: Response): Promise<T> {
  ...
  if (contentType.toLowerCase().includes('application/json')) {
    ...
    if (!response.ok) {
      throw new ApiError(response.status, payload, `Request failed with ${response.status}`);
    }
    return payload as T;
  }
...
function ManagementApp() {
  ...
  if (loading) return <PageStatus>Loading your authorized management workspace…</PageStatus>;
...
function StudentsHeader() {
  return <AppShell current="students" csrfToken={csrfToken} />;
}
...
function StudentsApp() {
  ...
  if (loading) return <><StudentsHeader /><PageStatus>Loading authorized student and admissions records…</PageStatus></>;
...
function StudentDetail({ lifecycle, branches, people, permissionOptions, onBack, onChanged, onError, error, message }: { lifecycle: StudentLifecycle; branches: BranchOption[]; people: RegistrationPerson[]; permissionOptions: string[]; onBack: () => void; onChanged: (message: string) => void; onError: (message: string) => void; error: string | null; message: string | null }) {
  ...
  return <><StudentsHeader /><main id="workspace-main" className="workspace" aria-labelledby="student-detail-title"><div className="detail-back"><button className="text-button" type="button" onClick={onBack}>← Back to student directory</button></div><header className="workspace-header"><div><p className="eyebrow">Canonical learner lifecycle</p><h1 id="student-detail-title">{lifecycle.person.legal_name}</h1><p className="lede">{lifecycle.student_code} · {lifecycle.person.verified ? 'Verified identity' : 'Identity verification requires attention'}</p></div><div className="detail-header-actions"><span className="status-chip">{humanize(lifecycle.status ?? 'status unavailable')}</span><a className="button secondary" href={`/print/id-card/${encodeURIComponent(lifecycle.student_id)}`}>Print ID card</a></div></header>
    ...
    <div className="student-detail-grid"><div className="student-detail-main">
      ...
      {lifecycle.capabilities.finance && (
        <section className="panel">
          <div className="section-heading"><div><p className="eyebrow">Finance-owned facts</p><h2>Obligations and payments</h2></div><span className="source-note">Read-only Finance projection · no money is created here</span></div>
          <h3>Obligations</h3>
          {lifecycle.obligations.length === 0 ? <p className="empty">No visible Finance obligations.</p> : (
            <ul className="fact-list">
              {lifecycle.obligations.map((item) => (<li key={item.id}><strong>{item.original_amount} · {humanize(item.source)}</strong><span>{item.reason} · {item.current_home_branch_id || item.originating_branch_id || 'provenance unavailable'}</span></li>))}
            </ul>
          )}
          <h3>Payments</h3>
          {lifecycle.payments.length === 0 ? <p className="empty">No visible Finance payments.</p> : (
            <ul className="fact-list">
              {lifecycle.payments.map((item) => (<li key={item.id}><strong>{item.amount} · {humanize(item.method)}</strong><span>{item.received_on} · {item.current_home_branch_id || item.originating_branch_id || 'provenance unavailable'}</span></li>))}
            </ul>
          )}
        </section>
      )}
...
function App() {
  ...
  if (loading) return <PageStatus>Loading your authorized workspace…</PageStatus>;

  return (
    <>
      <AppShell current="workspace" csrfToken={csrfToken} />
      <main id="workspace-main" className="workspace" aria-labelledby="workspace-title">
      ...
      <section aria-label="Common work areas">
        <div className="section-heading"><div><p className="eyebrow">Quick access</p><h2>Move to the work</h2></div><span className="source-note">Navigation follows the operator's work, not the database.</span></div>
        <div className="quick-actions">
          <a className="quick-action" href="/students"><span className="quick-icon" aria-hidden="true"><Icon name="users" /></span><span><strong>Students & admissions</strong><small>Directory, applications and lifecycle</small></span></a>
          <a className="quick-action" href="/academic"><span className="quick-icon" aria-hidden="true"><Icon name="academic" /></span><span><strong>Academic operations</strong><small>Classes, capacity, sessions and outcomes</small></span></a>
          <a className="quick-action" href="/crm"><span className="quick-icon" aria-hidden="true"><Icon name="crm" /></span><span><strong>CRM</strong><small>Visitors, follow-up and conversations</small></span></a>
          <a className="quick-action" href="/finance"><span className="quick-icon" aria-hidden="true"><Icon name="finance" /></span><span><strong>Finance</strong><small>Source-owned monetary operations</small></span></a>
        </div>
      </section>
...
if (root) {
  ...
  createRoot(root).render(view === 'academic' ? <AcademicApp getJson={getJson} postJson={postJson} csrfToken={csrfToken} /> : view === 'teachers' ? <TeacherApp getJson={getJson} postJson={postJson} csrfToken={csrfToken} /> : view === 'crm' ? <CrmApp /> : view === 'management' ? <ManagementApp /> : view === 'students' ? <StudentsApp /> : <App />);
}
