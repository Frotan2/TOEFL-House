...
export function SegmentedTabs<T extends string>({ items, value, onChange, ariaLabel }: { items: Array<{ value: T; label: string; count?: number }>; value: T; onChange: (value: T) => void; ariaLabel: string }) {
  return <div className="segmented-tabs" role="tablist" aria-label={ariaLabel}>{items.map((item) => <button key={item.value} className={value === item.value ? 'active' : ''} role="tab" aria-selected={value === item.value} type="button" onClick={() => onChange(item.value)}>{item.label}{item.count !== undefined && <span>{item.count}</span>}</button>)}</div>;
}
