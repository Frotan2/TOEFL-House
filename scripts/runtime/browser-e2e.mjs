...
const CONSOLES = [
  ['/workspace', 'Workspace'], ['/students', 'Students'], ['/academic', 'Academic'], ['/teachers', 'Teacher'], ['/crm', 'CRM'],
  ['/management', 'Management'], ['/reporting', 'Reporting'], ['/finance', 'Finance'], ['/hr', 'HR'], ['/payroll', 'Payroll'],
  ['/placement', 'Placement'], ['/access', 'Access'], ['/organization', 'Organization'], ['/identity', 'Identity'],
  ['/governance/privacy', 'Privacy'], ['/governance/audit', 'Audit'],
];
const results = [];
...
let health;
try { health = await fetch(`${BASE}/health`, { signal: controller.signal }); }
catch (error) { console.error(`Cannot reach ${BASE}/health: ${error instanceof Error ? error.message : String(error)}`); process.exit(1); }
finally { clearTimeout(timeout); }
record('Health endpoint is reachable', health.ok, `HTTP ${health.status}`);
if (!health.ok) process.exit(1);

const browser = await puppeteer.launch({ executablePath: EXECUTABLE, args: ['--no-sandbox', '--disable-dev-shm-usage', '--disable-gpu'], headless: true });
try {
  const page = await browser.newPage();
  await page.setViewport({ width: 1440, height: 1000, deviceScaleFactor: 1 });
  page.setDefaultNavigationTimeout(30_000); page.setDefaultTimeout(10_000);
  const consoleErrors = []; const failedRequests = []; const apiCalls = [];
  page.on('console', (message) => { if (message.type() === 'error') consoleErrors.push(`${page.url()} :: ${message.text().slice(0, 200)}`); });
  ...
  page.on('response', (response) => { const url = response.url(); if (url.includes('/api/v1/')) apiCalls.push({ url: url.replace(BASE, ''), status: response.status() }); if (response.status() >= 400 && !url.includes('favicon')) failedRequests.push(`${response.status()} ${url.replace(BASE, '').slice(0, 140)}`); });
  ...
  if (!authenticated) throw new Error('browser E2E authentication failed');

  for (const [path, expected] of CONSOLES) {
    const beforeErrors = consoleErrors.length; const beforeFailures = failedRequests.length;
  ...
  record('Frontend uses canonical API successfully', apiCalls.length > 0 && badCalls.length === 0, `${apiCalls.length} API calls observed; ${badCalls.length} failed`);
