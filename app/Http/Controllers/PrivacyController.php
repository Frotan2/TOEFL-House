...
/** Legacy write transport; canonical privacy reads live in the React/API surface. */
final class PrivacyController extends Controller
{
    ...
    public function recordConsent(Request $request): RedirectResponse
    {
        $input = $request->validate([
            'subject_person_id' => ['required', 'string'], 'purpose_id' => ['required', 'string'], 'evidence_ref' => ['required', 'string', 'max:500'],
    ...
    {
        $input = $request->validate([
            'subject_person_id' => ['required', 'string'], 'recipient' => ['required', 'string', 'max:200'], 'purpose' => ['required', 'string', 'max:500'],
            'authority' => ['required', 'string', 'max:120'], 'scope_type' => ['required', 'in:organization,campus,branch,department,subject'],
