...
final class PayrollApiController extends Controller
{
    ...
    {
        $this->requireOrganizationRead('payroll.period', 'api.payroll.workspace');
        ...
        return response()->json(['data' => [
            ...
            'employments' => Employment::query()->whereIn('id', $employmentIds)->orderBy('id')->limit(300)->get(),
            'terminated_employments' => Employment::query()->where('lifecycle_state', 'terminated')->whereIn('id', $employmentIds)->orderBy('id')->limit(200)->get(),
            'clearances' => PayrollClearance::query()->whereIn('employment_id', $employmentIds)->orderBy('id')->limit(500)->get(),
            'settlement_proposals' => SettlementProposal::query()->where('lifecycle_state', SettlementProposal::STATE_PROPOSED)->whereIn('employment_id', $employmentIds)->orderBy('id')->limit(200)->get(),
        ]]);
    }
